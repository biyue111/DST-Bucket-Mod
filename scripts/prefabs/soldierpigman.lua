...
local function ShouldAcceptItem(inst, item)
    ...
    elseif item.components.edible ~= nil then
		...
		elseif item.components.edible.foodtype == FOODTYPE.ELEMENTAL and item.components.edible.hungervalue == 2 then
			return inst.components.follower.leader == nil or inst.components.follower:GetLoyaltyPercent() <= 0.9
...
local function OnGetItemFromPlayer(inst, giver, item)
	if item.components.edible ~=nil and item.components.tradable ~= nil then
	-- Follow Wilson when given gold
...
local normalbrain = require "brains/pigmansoldierbrain"
...
local function SetNormalPig(inst)
	...
	-- wear footballhat at the beginning
	local footballhat = SpawnPrefab("footballhat")
...
local guardbrain = require "brains/pigmansoldierbrain"
...
return Prefab("soldierpigman", normal, assets, prefabs)
